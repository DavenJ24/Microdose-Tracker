...
function load(){try{const raw=localStorage.getItem(STORAGE_KEY);if(raw)return JSON.parse(raw);}catch(e){console.warn(e)};
  const now=new Date().toISOString();
  const d={meta:{appVersion:'1.2.0',createdAt:now,updatedAt:now},participant:{initials:'',age:null,sexOrGender:'',handDominance:'right',heightCm:null,weightKg:null,goals:[],protocol:{type:'fadiman',pattern:[1,0,0,1,0,0,1]}},baseline:null,doses:[],daily:[],weekly:[],ftt:[],pvt:[]};
  save(d);return d;
}
...
function updateKpisAndCharts(){
  ...
  const mood=daily.map(d=>+d.mood||null);
  const anx=daily.map(d=>+d.anxiety||null);
  const stress=daily.map(d=>+d.stress||null);
  const sleep=daily.map(d=>+d.sleepHours||null);

  makeOrUpdateLine('chartMood','Mood (1–10)',labels,mood,'#7ee787');
  makeOrUpdateLine('chartAnxiety','Anxiety (0–10)',labels,anx,'#ef7f7f');
  makeOrUpdateLine('chartStress','Stress (0–10)',labels,stress,'#ffd166');
  makeOrUpdateLine('chartSleep','Sleep hours',labels,sleep,'#7db3ff');
...
function makeOrUpdateMultiline(id, title, labels, series){
  if(!chartsCache[id]){
    chartsCache[id]=new Chart(el('#'+id),{type:'line',data:{labels,datasets:series.map(s=>({label:s.label,data:s.data,borderColor:s.color,pointRadius:3,tension:.2}))},options:{plugins:{},responsive:true,maintainAspectRatio:false}});
  }else{
...
/* ---- Insight generator (plain‑language trend read) ---- */
function linRegSlope(nums){
  const x=nums.map((_,i)=>i+1), y=nums.map(v=>+v||0);
  const n=y.length; if(n<2) return 0;
  const sx=sum(x), sy=sum(y), sxx=sum(x.map(v=>v*v)), sxy=sum(x.map((v,i)=>v*y[i]));
  const denom = n*sxx - sx*sx; if(denom===0) return 0;
  return (n*sxy - sx*sy)/denom; // slope per day
}
...
function generateInsightText(){
  ...
  if(mSlope>0.1 && aSlope<-0.1){ lines.push('Combination of rising mood and falling anxiety often signals a favorable short‑term response. Keep routines steady and keep logging.'); }
  if(sSlope<-0.1 && slSlope>0.1){ lines.push('Lower stress with improving sleep usually move together; consider anchoring a consistent wind‑down routine on dose days and off days.'); }
  if(Math.abs(mSlope)<0.05 && Math.abs(aSlope)<0.05 && Math.abs(sSlope)<0.05){ lines.push('Most signals are steady. That’s useful: stability helps you notice subtler changes when they begin.'); }

  lines.push('<span class="muted small">Guidance only. These are patterns, not diagnoses. If your symptoms worsen or you have safety concerns, seek professional support.</span>');
...
/* ----- Baseline / Weekly scales builders ----- */
...
function handleBaselineSave(){
  ...
  const phq=getVals('phq9q',phq9.length), gad=getVals('gad7q',gad7.length), pss=getVals('pss10q',pss10.length), who=getVals('who5q',who5.length);
  ...
  save();
  el('#baselineSummary').classList.remove('hidden');
...
function handleWeeklySave(){
  ...
  const phq=getVals('weeklyPhq',phq9.length), gad=getVals('weeklyGad',gad7.length), pss=getVals('weeklyPss',pss10.length), who=getVals('weeklyWho',who5.length);
...
function initNav(){el('#bottomNav').addEventListener('click',e=>{if(e.target.tagName==='BUTTON')show(e.target.dataset.target);});}
